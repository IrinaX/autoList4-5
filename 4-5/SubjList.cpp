...
{
	switch (pCar->type) {
	case CarType::itPassengerCar: {
		((PassengerCar*)pCar)->print();
		break;
	}
	case CarType::itMicroBus: {
		((MicroBus*)pCar)->print();
		break;
	}
	case CarType::itBus: {
	...
	case CarType::itTruck: {
		((Truck*)pCar)->print();
		break;
	}
	case CarType::itSpecial: {
		((Special*)pCar)->print();
		break;
	}
	default: {
...
void SubjList::placeSort()
{
	...
	for (int i = 0; i < amount; ++i) {//сортируем методом пузырьковой сортировки
		for (int j = 0; j < amount - i - 1; ++j) {
			MicroBus* pCurrMicroBus = (MicroBus*)getItem(j);//MicroBus так как Bus наследуется от MicroBus
			MicroBus* pNextMicroBus = (MicroBus*)getItem(j + 1);//MicroBus так как Bus наследуется от MicroBus
